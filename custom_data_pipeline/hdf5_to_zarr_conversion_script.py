...
def build_lowdim_episode(
    ...
    for key in required_keys:
        if key not in h5_file:
            raise KeyError(f"{h5_file.filename}: missing /{key}")
    ...
    if eef_pos.ndim != 2 or eef_pos.shape[1] != 3:
        raise ValueError(f"{h5_file.filename}: /obs/eef_pos must be (T, 3), got {eef_pos.shape}")
    if eef_quat.ndim != 2 or eef_quat.shape[1] != 4:
        raise ValueError(f"{h5_file.filename}: /obs/eef_quat must be (T, 4), got {eef_quat.shape}")
    if gripper_qpos.ndim != 2 or gripper_qpos.shape[1] < 1:
        raise ValueError(f"{h5_file.filename}: /obs/gripper_qpos must be (T, >=1), got {gripper_qpos.shape}")

    episode_length = eef_pos.shape[0]
    if episode_length == 0:
        raise ValueError(f"{h5_file.filename}: empty episode")
    if eef_quat.shape[0] != episode_length or gripper_qpos.shape[0] != episode_length:
        raise ValueError(
            f"{h5_file.filename}: lowdim lengths do not match "
    ...
    if np.any(quat_norm <= 1e-8):
        raise ValueError(f"{h5_file.filename}: /obs/eef_quat contains near-zero quaternion")
...
def append_lowdim_episodes(
    ...
    for path in tqdm(hdf5_paths, desc="lowdim episodes"):
        with h5py.File(path, "r") as h5_file:
            ...
            if camera_key not in h5_file:
                raise KeyError(f"{path}: missing /{camera_key}")
            camera = h5_file[camera_key]
            if not isinstance(camera, h5py.Dataset):
                raise TypeError(f"{path}: /{camera_key} is not a dataset")
            if camera.ndim != 4 or camera.shape[-1] != 3:
                raise ValueError(f"{path}: /{camera_key} must be (T, H, W, 3), got {camera.shape}")
            if camera.shape[0] != episode_length:
                raise ValueError(
                    f"{path}: /{camera_key} length {camera.shape[0]} != lowdim length {episode_length}"
                )
            if camera.dtype != np.uint8:
                raise ValueError(f"{path}: /{camera_key} must be uint8, got {camera.dtype}")
...
def write_camera0_rgb(
    ...
    for spec in tqdm(image_specs, desc="image episodes"):
        ...
        with h5py.File(path, "r") as h5_file:
            ...
            if camera.shape[0] != episode_length:
                raise ValueError(
                    f"{path}: /{camera_key} length {camera.shape[0]} != image spec length {episode_length}"
                )
